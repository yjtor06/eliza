...
class Eliza:
        ...
        with open(path) as file:
            for line in file:
                ...
                if tag == 'initial':
                    ...
                    key = Key(word, weight, [])
                    self.keys[word] = key
                    print(f"Loaded key: {word} with weight: {weight}")
                elif tag == 'decomp':
                    parts = content.split(' ')
                    save = False
                    if parts[0] == '$':
    ...
    def respond(self, text):
        ...
        for key, key_obj in self.keys.items():
            ...
            for decomp in key_obj.decomps:
                #print(decomp.parts)
                if self._match_decomp(decomp.parts, words):  # Supposant que _match_decomp retourne True si wordc correspond à decomp.parts
                    keys.append(key_obj)
                    print(key_obj.word,key_obj.weight)
                    break  # Arrêtez de chercher d'autres décompositions pour cette clé si une correspondance est trouvée
        
        #keys = [self.keys[w.lower()] for w in wordc if w.lower() in self.keys]
